% Plots AVERAGE  POWER of splines as points for all sessions on TIME of the day

% !NOTE! you should have cronux added to the path (folders and subfolders)
% for the spectrogram -thus the spectrum--> freq of splines 110-160 Hz-
% function
...
ALLDUR=[] ;%This array will store all the durations of the all the rem stages of all the sessions for the mouse in case you want to have access to that to plot the histograms
%I placed it in this script and not the splines ones because this one is
%faster 
...
ALLDUR=[ALLDUR, duration] ;%in case you need that for the histograms or for anything
    ...
    for i=1:length(start)
       ...
       %From the spectrum we get to see which points are into the spline
       %range (110-160 Hz) and for those points we get the mean power of
       %the splines
    ...
    title( ['Rem Stages / Duration of ' int2str(sesnum) ' Sessions for Mouse: ' Mo 'used channel'  int2str(channel_num) ':'] )%, num2str(su))
